...
namespace RailcannonPod
{
    internal static class Hud
    {
        ...
        internal static void Tick()
        {
            ...
            bool loaded = Pod.AircraftHasPod(_ac);
            bool selected = loaded && Pod.CurrentStationIsOurs(_ac);
            _has = selected;
            if (loaded)
                Pod.StampCurrentGun(_ac);
            if (!_loggedOnce)
            {
                _loggedOnce = true;
                if (Plugin.Log != null)
            ...
            if (loaded)
            {
                PowerSupply psTick = Pod.PowerOf(_ac);
                Pod.EnsureCapacitor(_ac, psTick);
                Pod.TickBattery(_ac, psTick, Time.unscaledDeltaTime);
            }
            if (!selected)
            {
            ...
            PowerSupply ps = Pod.PowerOf(_ac);
            EnsureUi();
            UpdatePanel(_ac, ps);
            Impact.Tick(_ac);
            UpdatePip();
        ...
        private static void PollKeys(bool fromGui)
        {
            ...
            if (Time.frameCount == _toggleFrame)
                return;
            if (_ac == null || !Pod.CurrentStationIsOurs(_ac))
                return;
            _toggleFrame = Time.frameCount;
            Pod.CycleMode();
            string msg = Pod.DisplayName + " " + Pod.ModeLabel();
            _flash = msg;
            ...
            if (Plugin.Log != null)
                Plugin.Log.LogInfo("57mm Railgun mode=" + Pod.ModeLabel());
            Pod.StampCurrentGun(_ac);
        }
        ...
        private static void LogGuns(Aircraft ac)
        {
            ...
            Plugin.Log.LogInfo("no railgun guns=" + n);
            if (guns == null)
                return;
            int max = n < 12 ? n : 12;
            for (int i = 0; i < max; i++)
        ...
        private static void UpdatePanel(Aircraft ac, PowerSupply ps)
        {
            ...
            float next = Pod.PeekShotCost(ac, ps);
            float have = Pod.ReadKJ(ac, ps);
            float vel = Pod.PeekMuzzle(ac, ps);
            string mode = Pod.ModeLabel();
            string last = Pod.LastShotKJ > 0f ? FormatKj(Pod.LastShotKJ) : "--";
            _panel.color = Pod.ModeColor();
            string shotExtra = "";
            if (Pod.ChargeMode)
                shotExtra = "  ALL";
            else if (Pod.GuidedMode)
                shotExtra = "  OPT";
            _panel.text = Pod.DisplayName + "  " + mode
        ...
        private static void EnsureUi()
        {
            ...
            {
                if (_canvas.gameObject.name != "HUD_57mmRailgun")
                {
            ...
            GameObject root = new GameObject("HUD_57mmRailgun");
            ...
            prt.sizeDelta = new Vector2(380f, 140f);
