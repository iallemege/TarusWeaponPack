...
namespace RailcannonPod
{
    ...
    {
        private const int MaxSteps = 360;
        ...
        {
            if (ac == null || ac.disabled || !Pod.CurrentStationIsOurs(ac))
            {
                _has = false;
                return;
            }
        ...
        private static void Compute(Aircraft ac)
        {
            ...
            PowerSupply ps = Pod.PowerOf(ac);
            float muzzle = Pod.PeekMuzzle(ac, ps);
            vel += fwd * muzzle;
            _muzzleWorld = pos;
