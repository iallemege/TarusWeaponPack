...
namespace RailcannonPod
{
    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        internal static ConfigEntry<bool> AddToAllHardpoints;
        ...
        {
            Log = log;
            AddToAllHardpoints = cfg.Bind(configSection, "AddToAllHardpoints", true,
                "Put the 57mm Railgun on every aircraft hardpoint (Aryx included).");
            ChargeToggleKey = cfg.Bind(configSection, "ChargeToggleKey", KeyCode.K,
                "Cycle 57mm Railgun NORMAL / CHARGE / GUIDED. Same key as 106mm; only the selected gun cycles.");
            ...
            Log.LogInfo("CombatHUD.LateUpdate="
                + (AccessTools.Method(typeof(CombatHUD), "LateUpdate") != null)
                + " Gun.SpawnBullet="
                + (AccessTools.Method(typeof(Gun), "SpawnBullet", new Type[] { typeof(float) }) != null));
            Log.LogInfo(PackInfo.Name + " v" + PackInfo.Version
                + " 57mm Railgun HUD, mode key=" + ChargeToggleKey.Value);
        }
        ...
        internal static void TickFrame()
        {
            ...
            {
                if (Log != null)
                    Log.LogWarning("RailcannonPod hud: " + ex.Message);
            }
            try { LaunchFx.Warm(); }
            catch { }
            if (Time.unscaledTime < _nextEnsure)
                return;
            _nextEnsure = Time.unscaledTime + (Pod.Injected ? 20f : 0.5f);
            try
            {
                Pod.Ensure();
            }
            ...
            {
                if (Log != null)
                    Log.LogWarning("RailcannonPod ensure: " + ex.Message);
            }
        }
        ...
        internal static void DrawFrame()
        {
            ...
            {
                if (Log != null)
                    Log.LogWarning("RailcannonPod OnGUI: " + ex.Message);
            }
        }
        ...
        internal static bool InMission()
        {
            try
            {
                return GameManager.gameState.IsSingleOrMultiplayer();
            }
            catch
            {
                return false;
