...
namespace RailcannonPod
{
    ...
    {
        private const string FileName = "57mmRailgun_icon.png";
        ...
        internal static Sprite Get(Sprite donor)
        {
            ...
            if (_tried)
                return null;
            _tried = true;
            string path = HaloVisual.ResolveAssetPath(FileName);
            if (string.IsNullOrEmpty(path))
            {
                if (Plugin.Log != null)
                    Plugin.Log.LogWarning("57mm Railgun icon PNG missing");
                return null;
            }
            try
            {
                ...
                _sprite = Sprite.Create(
                    ...
                    ppu);
                _sprite.name = "57mmRailgun_Icon";
                if (Plugin.Log != null)
                    Plugin.Log.LogInfo("57mm Railgun icon " + fitted.width + "x" + fitted.height);
                return _sprite;
            }
            catch (Exception ex)
            {
                if (Plugin.Log != null)
                    Plugin.Log.LogWarning("57mm Railgun icon: " + ex.Message);
                return null;
            }
        }
