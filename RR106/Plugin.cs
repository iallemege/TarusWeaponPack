    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        internal static void Boot(ManualLogSource log, ConfigFile cfg, string configSection)
        {
            ...
            AddToAllHardpoints = cfg.Bind(configSection, "AddToAllHardpoints", true,
                "Put the 106mm Recoilless Rifle on every aircraft hardpoint.");
            ChargeToggleKey = cfg.Bind(configSection, "ChargeToggleKey", KeyCode.K,
                "Cycle 106mm NORMAL / CHARGE / GUIDED / RECON / CHG+GUIDE. Same key as 57mm; only the selected gun cycles.");
            ...
            Log.LogInfo(PackInfo.Name + " v" + PackInfo.Version
                + " standalone 106mm Recoilless Rifle, 50 kJ/shot, mode key="
        ...
        internal static void TickFrame()
        {
            ...
            {
                if (Log != null)
                    Log.LogWarning("RR106 hud: " + ex.Message);
            }
            if (Time.unscaledTime < _nextEnsure)
                return;
            _nextEnsure = Time.unscaledTime + (Rifle.Injected ? 20f : 0.5f);
            try
            {
                Rifle.Ensure();
            }
            ...
            {
                if (Log != null)
                    Log.LogWarning("RR106 ensure: " + ex.Message);
            }
        }
        ...
        internal static void DrawFrame()
        {
            ...
            {
                if (Log != null)
                    Log.LogWarning("RR106 OnGUI: " + ex.Message);
            }
        }
        ...
        internal static T TryAddBehaviour<T>(GameObject go) where T : MonoBehaviour
        {
            ...
            {
                if (Log != null)
                    Log.LogWarning("RR106 AddComponent " + typeof(T).Name + ": " + ex.Message);
                return null;
            }
        }
    }
