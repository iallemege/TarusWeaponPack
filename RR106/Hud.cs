...
namespace RR106
{
    internal static class Hud
    {
        ...
        internal static void Tick()
        {
            ...
            bool loaded = Rifle.AircraftHasPod(_ac);
            bool selected = loaded && Rifle.CurrentStationIsOurs(_ac);
            _has = selected;
            if (loaded)
                Rifle.StampCurrentGun(_ac);
            if (!_loggedOnce)
            {
                _loggedOnce = true;
                if (Plugin.Log != null)
                    Plugin.Log.LogInfo("HUD tick ac=" + _ac.name
                        + " loaded=" + loaded
                        + " selected=" + selected
                        + " mode=" + Rifle.ModeLabel());
            }
            if (loaded)
            {
                PowerSupply psTick = Rifle.PowerOf(_ac);
                Rifle.EnsureCapacitor(_ac, psTick);
                Rifle.TickBattery(_ac, psTick, Time.unscaledDeltaTime);
            }
            if (!selected)
            {
            ...
            PowerSupply ps = Rifle.PowerOf(_ac);
            EnsureUi();
            UpdatePanel(_ac, ps);
            Impact.Tick(_ac);
            UpdatePip();
        ...
        private static void PollKeys(bool fromGui)
        {
            ...
            if (Time.frameCount == _toggleFrame)
                return;
            if (_ac == null || !Rifle.CurrentStationIsOurs(_ac))
                return;
            _toggleFrame = Time.frameCount;
            Rifle.CycleMode();
            string msg = Rifle.DisplayName + " " + Rifle.ModeLabel();
            _flash = msg;
            ...
            if (Plugin.Log != null)
                Plugin.Log.LogInfo("106mm mode=" + Rifle.ModeLabel());
            Rifle.StampCurrentGun(_ac);
        }
        ...
        private static void LogGuns(Aircraft ac)
        {
            ...
            Plugin.Log.LogInfo("no 106mm guns=" + n);
            if (guns == null)
                return;
            int max = n < 12 ? n : 12;
            for (int i = 0; i < max; i++)
        ...
        private static void UpdatePanel(Aircraft ac, PowerSupply ps)
        {
            ...
            float next = Rifle.PeekShotCost(ac, ps);
            float have = Rifle.ReadKJ(ac, ps);
            float vel = Rifle.PeekMuzzle(ac, ps);
            string mode = Rifle.ModeLabel();
            string last = Rifle.LastShotKJ > 0f ? FormatKj(Rifle.LastShotKJ) : "--";
            _panel.color = Rifle.ModeColor();
            string shotExtra = "";
            if (Rifle.ChargeMode || Rifle.ChargeGuidedMode)
                shotExtra = "  ALL";
            else if (Rifle.GuidedMode)
                shotExtra = "  OPT";
            else if (Rifle.ReconMode)
                shotExtra = "  RCN";
            string range = Rifle.PeekRangeKm().ToString("F0") + " km";
            _panel.text = Rifle.DisplayName + "  " + mode
                + "\nSHOT  " + FormatKj(next) + shotExtra
                + "\nVEL   " + vel.ToString("F0") + " m/s"
                + "\nRANGE " + range
                + "\nLAST  " + last
                + "\nBATT  " + FormatKj(have)
        ...
        {
            if (_pip == null || _pipRt == null)
                return;
            Vector3 sp;
            if (!Impact.TryScreenPoint(out sp))
            {
                SetPipVisible(false);
                return;
            }
            SetPipVisible(true);
            _pip.color = Rifle.ModePipColor();
            _pipRt.position = new Vector3(sp.x, sp.y, 0f);
        }
        ...
        private static void EnsureUi()
        {
            ...
            {
                if (_canvas.gameObject.name != "HUD_RR106")
                {
            ...
            GameObject root = new GameObject("HUD_RR106");
