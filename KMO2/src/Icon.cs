...
namespace KMO2
{
    ...
    {
        private const string FileName = "KMO2_icon.png";
        ...
        internal static Sprite Get(Sprite donor)
        {
            ...
            {
                if (Plugin.Log != null)
                    Plugin.Log.LogWarning("KMO-2 icon PNG missing");
                return null;
            }
            try
            {
                ...
                if (!ImageConversion.LoadImage(tex, bytes, false))
                {
                    ...
                    if (Plugin.Log != null)
                        Plugin.Log.LogWarning("KMO-2 icon decode failed");
                    return null;
                }
                ...
                _sprite = Sprite.Create(
                    ...
                    ppu);
                _sprite.name = "KMO2_Icon";
                if (Plugin.Log != null)
                    Plugin.Log.LogInfo("KMO-2 icon " + fitted.width + "x" + fitted.height);
                return _sprite;
            }
            catch (Exception ex)
            {
                if (Plugin.Log != null)
                    Plugin.Log.LogWarning("KMO-2 icon: " + ex.Message);
                return null;
            }
        }
